...
var cfg=require(process.cwd()+"/config/cloud-cfg.js");
var mongodb=require("mongodb");

if (!mongodb.Db.connect){
...
Set.prototype.save = function(fn){
  var self = this
    ...
    mongodb.connect(cfg.mongourl, {}, function(err, db){
        var migration=db.collection("migration");

        migration.findOne(function(err, doc){
        if (!doc) {
          var m = { migration: self }
          migration.save(m, cb);
        } else {
          doc.migration = self
          migration.save(doc, cb);
        }
      });
    });
...
Set.prototype._migrate = function(direction, fn, migrationName){
  ...
  function next(err, db, migration) {
    ...
    migration[direction](db, function(err){
      next(err, db, migrations.shift());
    });
  }
    mongodb.connect(cfg.mongourl, {}, function(err, db){
        next(null, db, migrations.shift());
    });
};
